...
$(".side-menu").on('click', 'li a', function(e) {
	...
	var unclick = $this.parent("li").attr("unclick");
	//禁止导航点击
	if(unclick) {
		//防止有链接跳转
		e.preventDefault();
...
function getNavigatioBar(){
	ajax("/api/config",{},'json',function(data){
		userName(data)
		initMenuByData(data.result);
	});
}
function userName(data){
	$("#userName").text(data.result.user.userName)
	$(".header-bar-role a").text(data.result.roleList[0].roleName)
}

function initMenuByData(data){
	var menu = data.moduleList
